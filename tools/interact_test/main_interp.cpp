...
int main(int argc, char** argv)
{
    ...
    double step_perc = 1;
    cxxopts::Options options("Interact-Test [Interpolation]", "");
    options.add_options()("r,res_file", "The results file to process", cxxopts::value(res_file))(
        "s,step", "The step size in percent (default 1)", cxxopts::value(step_perc))("o,out",
    ...
    for (std::size_t i = 0; i < upper_bound; i += step)
    {
        ...
        detections.save(
            out_dir / ("detections_int_" + std::to_string(manual_uns.size()) + ".yaml"));
        progress.inc();
    }
    progress.done();
