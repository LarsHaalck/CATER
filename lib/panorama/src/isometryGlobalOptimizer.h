...
public:
        ...
        const T* const isoVec1, T* errors) const
    {
        ...
        T angle1 = isoVec1[0];
        Mat3 isoMat0;
        isoMat0 << cos(angle0), sin(angle0), isoVec0[1], -sin(angle0), cos(angle0), isoVec0[2],
            T(0), T(0), T(1);

        Mat3 isoMat1;
        isoMat1 << cos(angle1), sin(angle1), isoVec1[1], -sin(angle1), cos(angle1), isoVec1[2],
            T(0), T(0), T(1);
        ...
        Mat3 transformation = camMat * isoMat1.inverse() * isoMat0 * camMat.inverse();
