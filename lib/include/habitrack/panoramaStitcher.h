...
public:
    PanoramaStitcher(std::shared_ptr<ImageContainer> imgContainer,
        ...
        std::shared_ptr<MatchesContainer> matchContainer,
...
private:
    ...
    cv::Rect2d generateBoundingRect(const std::vector<cv::Mat>& trafos,
        const cv::Size& imageSize);
    cv::Rect2d generateBoundingRectHelper(const cv::Mat& trafo, cv::Size size,
        cv::Rect2d currRect = cv::Rect2d());
