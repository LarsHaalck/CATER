...
{
public:
    IsometryGlobalFunctor(const Eigen::Vector2d& q1, const Eigen::Vector2d& q2);

    template <typename T>
    bool operator()(const T* const camParams, const T* const distParams,
        const T* const isoVec0, const T* const isoVec1, T* errors) const
    {
        (void)camParams;
        (void)distParams;
        ...
        T angle1 = isoVec1[0];
        Mat3 homMat0;
        homMat0 << cos(angle0), sin(angle0), isoVec0[1],
                  -sin(angle0), cos(angle0), isoVec0[2],
                  T(0), T(0), T(1);

        Mat3 homMat1;
        homMat1 << cos(angle1), sin(angle1), isoVec1[1],
                  -sin(angle1), cos(angle1), isoVec1[2],
                  T(0), T(0), T(1);
        ...
        Mat3 camMat;
        camMat << camParams[0], T(0), camParams[2],
                  T(0), camParams[1], camParams[3],
                  T(0), T(0), T(1);

        Mat3 transformation = camMat * homMat1.inverse() * homMat0 * camMat.inverse();
        symmetricReprojectionError<T>(transformation, mQ1T,
                mQ2T, errors);
        return true;
    }
private:
